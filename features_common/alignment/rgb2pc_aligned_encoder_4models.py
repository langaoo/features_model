"""features_common/alignment/rgb2pc_aligned_encoder_4models.py
...
说明
    - checkpoints: /home/gl/RoboTwin/policy/DP2DP3/features_model/outputs/train_rgb2pc_runs/.../ckpt_step_*.pt
...
class RGB2PCAligned4ModelSpec:
    n_models: int = 4
    in_dims: tuple[int, int, int, int] = (1024, 2048, 768, 2048)  # ✅ 修正: [CroCo, VGGT, DINOv3, DA3]
    fuse_dim: int = 1280
    fusion: str = "weighted"
        ...
        *,
        ...
        if use_context:
            ...
            if pos_sd is not None:
                ...
                ckpt_pe = pos_sd.get('pe')

                if ckpt_pe is not None:
                    # 💡 动态调整模型内部的 pe 缓冲区大小以匹配 Checkpoint
                    # 这样就不需要手动切片，后续 load_state_dict 就能 strict=True
                    if ckpt_pe.shape != model.pos_encoder.pe.shape:
                        print(f"[INFO] Adjusting PositionalEncoding: {model.pos_encoder.pe.shape} -> {ckpt_pe.shape}")
                        # 重新创建一个匹配形状的 buffer
                        model.pos_encoder.register_buffer('pe', torch.zeros_like(ckpt_pe))

                    model.pos_encoder.load_state_dict(pos_sd, strict=True)
                else:
                    # 形状匹配，正常加载
                    model.pos_encoder.load_state_dict(pos_sd, strict=False)
