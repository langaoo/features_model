...
- 输出：对齐后的 embedding 序列 z: FloatTensor[B, To, D]
    - output_mode="aligned": D == fuse_dim
    - output_mode="concat":  D == fuse_dim + in_dim（保留原始 RGB 向量）
    - output_mode="project": D == project_dim（对齐后再投影）
    - output_mode="concat_project": D == project_dim（拼接后再投影）
...
class RGB2PCStudentEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        output_mode = str(getattr(spec, "output_mode", "aligned")).lower()
        self.output_mode = output_mode

        base_out_dim = int(spec.fuse_dim)
        if output_mode in {"concat", "concat_project"}:
            base_out_dim = int(spec.fuse_dim) + int(spec.in_dim)

        project_dim = getattr(spec, "project_dim", None)
        self.project_dim = int(project_dim) if project_dim is not None else None

        if output_mode in {"project", "concat_project"}:
            if self.project_dim is None:
                raise ValueError("project_dim must be set when output_mode is project/concat_project")
            self.proj_out = nn.Sequential(
                nn.Linear(base_out_dim, int(self.project_dim)),
                nn.LayerNorm(int(self.project_dim)),
            )
            self.output_dim = int(self.project_dim)
        else:
            self.proj_out = None
            self.output_dim = int(base_out_dim)
    ...
    def from_checkpoint(
        ...
        *,
        ...
        proj_out_sd = ckpt.get("proj_out")
        if proj_out_sd is not None and model.proj_out is not None:
            proj_out_sd = _strip_module_prefix(proj_out_sd)
            model.proj_out.load_state_dict(proj_out_sd, strict=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.output_mode in {"concat", "concat_project"}:
            z = torch.cat([x.to(dtype=z.dtype), z], dim=-1)

        if self.output_mode in {"project", "concat_project"}:
            if self.proj_out is None:
                raise RuntimeError("proj_out is not initialized")
            z = self.proj_out(z)
