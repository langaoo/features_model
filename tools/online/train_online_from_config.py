...
class DPRGBPolicy(nn.Module):
    ...
    def forward(self, obs):
        ...
        # 🔧 修复：限制动作范围（机械臂关节限位）
        # RoboTwin 环境的关节范围通常在 [-2.5, 2.5] rad
        action = torch.clamp(action, -2.5, 2.5)
...
def main():
    ...
    # 构建实际的数据路径（RoboTwin格式）
    # /home/gl/RoboTwin/data/{task_name}/{task_config}/data/episode{i}.hdf5
    ...
    expert_data_num = config.get('checkpoint', {}).get('expert_data_num', 20)
    
    # 构建数据路径
    task_name = config['data']['tasks'][0] if config['data']['tasks'] else 'beat_block_hammer'
    raw_data_root = os.path.join(robotwin_data_root, task_name, task_config, 'data')
    
    print(f"数据路径: {raw_data_root}")
    print(f"专家数据数量: {expert_data_num}")
    
    # 检查数据是否存在
    if not os.path.exists(raw_data_root):
        raise FileNotFoundError(f"数据目录不存在: {raw_data_root}")
    
    # 检查是否使用批量特征提取模式
    batch_extract = config['train'].get('batch_extract', False)
    
    dataset = DPRGBOnlineDataset(
        raw_data_root=raw_data_root,  # 使用前面计算出的路径变量
        ...
        assume_bgr=bool(config['data'].get('assume_bgr', True)),
    )
    ...
    for epoch in range(config['train']['epochs']):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # 数据加载耗时 (BatchCollateFn 包含特征提取，所以这部分其实包含了 feature_extract)
            # 为了精准统计，我们需要 breakdown collate_fn 里的时间，但这在 DataLoader 外部很难拿到
            # 除非我们修改 BatchCollateFn 返回时间信息。
            # 或者，我们只能统计总的 load_time (包含 extract)
            data_load_time = time.time() - batch_start

            # 为了区分 data_load 和 feature_extract，我们需要 hack 一下。
            # 但现在 data_load_time 确实主要由 feature_extract 占据
            ...
            if batch_idx < 100:  # 只统计前100个batch
                timing_stats['data_loading'].append(data_load_time)
                timing_stats['alignment'].append(align_time)
                timing_stats['forward'].append(forward_time)
                timing_stats['backward'].append(backward_time)
                timing_stats['total_per_batch'].append(batch_total_time)
            
            # 更新进度条
            pbar.set_postfix({
                'loss': f'{loss.item():.4f}',
                'time': f'{batch_total_time:.2f}s'
            })
        ...
        print(f"  Epoch耗时: {epoch_time:.1f}s ({epoch_time/60:.1f}分钟)")
        
        # 打印性能统计（仅第一个epoch）
        if epoch == 0 and len(timing_stats['total_per_batch']) > 0:
            print(f"\n[性能统计] (前100个batch的平均值)")
            print(f"  数据加载+特征提取: {np.mean(timing_stats['data_loading']):.3f}s (瓶颈通常在这里)")
            print(f"  对齐编码:       {np.mean(timing_stats['alignment']):.3f}s")
            print(f"  策略前向:       {np.mean(timing_stats['forward']):.3f}s")
            print(f"  反向传播:       {np.mean(timing_stats['backward']):.3f}s")
            print(f"  --------------------------------------------------")
            print(f"  单batch总耗时:   {np.mean(timing_stats['total_per_batch']):.3f}s")
            print(f"  预计单epoch耗时: {np.mean(timing_stats['total_per_batch']) * len(dataloader) / 60:.1f}分钟")
        ...
        if should_save:
            # 从配置中提取任务信息
            task_name = config['data']['tasks'][0] if config['data']['tasks'] else 'unknown_task'
            expert_data_num = config.get('checkpoint', {}).get('expert_data_num', 20)
            ckpt_setting = config.get('checkpoint', {}).get('ckpt_setting', 'demo_randomized')
            seed = config.get('checkpoint', {}).get('seed', 0)
            
            # 构建标准化路径：checkpoints/{task}-{setting}-{num}-{seed}/{epoch}.ckpt
            ckpt_dir_name = f"{task_name}-{ckpt_setting}-{expert_data_num}-{seed}"
            policy_root = Path(__file__).parent.parent.parent  # /home/gl/RoboTwin/policy/DP2DP3
            output_dir = policy_root / "checkpoints" / ckpt_dir_name
            output_dir.mkdir(parents=True, exist_ok=True)
            
            ckpt_epoch_num = epoch + 1
            ckpt_path = output_dir / f"{ckpt_epoch_num}.ckpt"
