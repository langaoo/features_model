#!/usr/bin/env python3
"""
4模型直接融合 + 本体感知(proprio) 训练 【离线版】
跳过对齐模块，加载离线4模型特征 → 融合 → 拼接agent_pos → Head → 动作

与 train_direct_fusion_offline.py 的区别：
- 新增 agent_pos 加载（来自 HDF5 的 joint_action/vector）
- agent_pos 采用 DP-aligned 方式：归一化后直接拼接到融合后的视觉特征
- 时间对齐：state[t]=vector[t], action[t]=vector[t+1]，共 T-1 有效帧
- normalizer 同时 fit action 和 agent_pos
"""
import torch
...
class DirectFusionProprioPolicy(nn.Module):
    """
    4模型直接融合 + proprio + Diffusion Policy
    
    与原版 DP 对齐的设计：
    - fusion_encoder 先将4模型特征融合为 [B, To, fuse_dim]
    - agent_pos 归一化后直接拼接到融合后特征
    - obs_encoder 输入 = n_obs_steps * (fuse_dim + proprio_dim)
    """
    ...
    def compute_loss(self, rgb_feats, agent_pos, actions):
        ...
        # 拼接 proprio
        obs_combined = torch.cat([fused, nagent_pos], dim=-1)  # [B, To, fuse_dim + proprio_dim]
...
class DirectFusionProprioDataset(Dataset):
    """
    4模型直融 + proprio 离线数据集
    ...
    def __init__(
        ...
        self.horizon = horizon
        self.n_obs_steps = n_obs_steps
        self.in_dims = [1024, 2048, 768, 2048]  # croco, vggt, dinov3, da3
        self.C_max = 2048
        self.M = 4
        ...
        print(f"[DirectFusion+proprio] 收集数据样本...")
        self.samples = self._collect_samples()
        print(f"[DirectFusion+proprio] 共 {len(self.samples)} 个样本")
...
def main():
    ...
    # 1. 创建数据集
    print("\n1. 创建数据集...")
    dataset = DirectFusionProprioDataset(
