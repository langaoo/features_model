#!/usr/bin/env python3
"""tools/depth_guided_film_online_3model/train_film_online_3model.py

半离线 3 模型 DA3-FiLM 训练 (DINOv3 + CroCo + DA3) — 快速版
=============================================================

核心流程:
  1. 加载 DINOv3 + CroCo + DA3 三个 backbone (完全冻结, no_grad)
...
与 2 模型版 (train_film_online.py) 的区别:
  - 增加 CroCo 语义模型
  - Encoder 使用 concat_proj 融合两个语义模型 (与离线 4 模型版一致)
  - 其他完全相同 (缓存策略, DP head, normalizer 等)
...
class DA3Film3ModelPolicy(nn.Module):
    """3 模型 DA3-FiLM + proprio + Diffusion Policy."""
    ...
    def compute_loss(self, tokens_list, agent_pos, actions):
        """
        tokens_list: List[3 × Tensor [B, To, K_i, C_i]]
        agent_pos:   [B, To, 14]
...
def _precompute_tokens(
    zarr_path: str,
    camera_name: str,
    extractors: ThreeModelExtractors,
    max_tokens: int,
    batch_size: int = 32,
) -> tuple:
    """
    从 zarr head_camera 批量提取 DINOv3 + CroCo + DA3 tokens, 缓存到 CPU RAM (float16).

    Returns:
        dino_tokens:  [N, K_d, 768]   float16 CPU
        croco_tokens: [N, K_c, 1024]  float16 CPU
    ...
    for i in tqdm(range(0, N, batch_size), desc="  预提取 tokens", leave=False):
        ...
        all_croco.append(tokens[1].half().cpu())
        all_da3.append(tokens[2].half().cpu())

    dino_all  = torch.cat(all_dino,  dim=0)   # [N, K_d, 768]
    croco_all = torch.cat(all_croco, dim=0)   # [N, K_c, 1024]
    da3_all   = torch.cat(all_da3,   dim=0)   # [N, K_a, 2048]
    mem_gb = (dino_all.nbytes + croco_all.nbytes + da3_all.nbytes) / 1e9
    print(f"  [precompute] DINOv3 {tuple(dino_all.shape)}, "
          f"CroCo {tuple(croco_all.shape)}, DA3 {tuple(da3_all.shape)}, "
          f"共 {mem_gb:.2f} GB (float16 CPU)")
    return dino_all, croco_all, da3_all
...
class CachedTokenDataset(Dataset):
    ...
    每个样本:
        ...
        croco_tokens: [To, K_c, 1024]  float16
    ...
    def __init__(
        ...
        self.croco_tokens = croco_tokens
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "croco_tokens": self.croco_tokens[obs_slice].float(),  # [To, K_c, 1024]
...
def main():
    parser = argparse.ArgumentParser(description="半离线 3 模型 DA3-FiLM 训练")
    ...
    print("=" * 65)
    print("半离线 3 模型 DA3-FiLM 训练 (DINOv3 + CroCo + DA3) — 快速版")
    print("=" * 65)
    ...
    # ---- 1. 加载 backbone (完全冻结) ----
    print("\n1. 加载 DINOv3 + CroCo + DA3 backbone (冻结)...")
    extractors = ThreeModelExtractors(gpu_id=gpu_id)
    for m in [extractors.dinov3_model, extractors.croco_model, extractors.da3_model]:
        m.requires_grad_(False)
        m.eval()
    print("   ✓ 三个 backbone 已冻结 (requires_grad=False, eval mode)")

    # ---- 2. 预提取所有帧 tokens ----
    print("\n2. 预提取所有帧 tokens (一次性, 之后训练不再调用 backbone)...")
    precompute_bs = int(config.get("precompute_batch_size", 32))
    dino_tokens, croco_tokens, da3_tokens = _precompute_tokens(
    ...
    # ---- 5. 模型 ----
    print("\n5. 创建 DA3Film3ModelEncoder + Policy ...")
    ...
    _enc_cfg_save = {
        "type":              "film_3model",
        "semantic_in_dims":  list(enc_cfg.get("semantic_in_dims", [768, 1024])),
        "geometric_in_dim":  int(enc_cfg.get("geometric_in_dim", 2048)),
        "proj_dim":          int(enc_cfg.get("proj_dim", 256)),
        "film_hidden":       int(enc_cfg.get("film_hidden", 256)),
        "out_dim":           int(enc_cfg.get("out_dim", 1280)),
    ...
    for epoch in range(total_epochs):
        ...
        for batch in pbar:
            tokens_list = [
                ...
                batch["croco_tokens"].to(device, non_blocking=True),  # [B, To, K_c, 1024]
        ...
        if (epoch + 1) % save_every == 0:
            ckpt = {
                ...
                "config":       config,
                "loss":         avg_loss,
                "policy_class": "DA3Film3ModelPolicy",
                "policy_type":  "depth_guided_film_online_3model",
