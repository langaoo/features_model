...
已迁移到 tools/features/ 目录下。可在 features_model 根目录以模块方式运行：
    python -m tools.features.run_extract_features --model croco
...
def main() -> None:
    ...
    p.add_argument(
        "--rgb_root", 
        type=str, 
        default="/home/gl/RoboTwin/policy/DP2DP3/features_model/rgb_dataset/RGB_ORI",
        help="RGB 图片根目录。可用路径：rgb_dataset/RGB（处理后）或 rgb_dataset/RGB_ORI（原始）"
    )
    p.add_argument(
        "--out_root",
        type=str,
        default="/home/gl/RoboTwin/policy/DP2DP3/features_model/rgb_dataset",
    ...
    # 模型特有参数（保持可选）
    p.add_argument("--croco_ckpt", type=str, default="/home/gl/RoboTwin/policy/DP2DP3/features_model/croco/pretrained_models/CroCo_V2_ViTLarge_BaseDecoder.pth")
    p.add_argument("--vggt_ckpt", type=str, default="/home/gl/RoboTwin/policy/DP2DP3/features_model/vggt/weight/model.pt")
    # dinov3 权重目录：需要指向包含 config.json 的具体子目录（如 B16/ 或 7B/）
    p.add_argument("--dinov3_weights", type=str, default="/home/gl/RoboTwin/policy/DP2DP3/features_model/dinov3/weight/B16")
    p.add_argument("--da3_model_dir", type=str, default="/home/gl/RoboTwin/policy/DP2DP3/features_model/Depth-Anything-3/weight")
    p.add_argument("--da3_export_layer", type=int, default=23)

    args = p.parse_args()

    # 统一输出命名：对齐你期望的结构
    # e.g. /home/gl/RoboTwin/policy/DP2DP3/features_model/rgb_dataset/features_vggt_encoder_dict_unified_zarr/<task>/<episode>.zarr
    ...
    if args.model == "croco":
        cmd = [
            "python",
            "/home/gl/RoboTwin/policy/DP2DP3/features_model/croco/extract_multi_frame_croco_features.py",
            "--dataset_root",
            args.rgb_root,
            "--output_root",
            model_out_root,
        ...
        cmd = [
            "python",
            "/home/gl/RoboTwin/policy/DP2DP3/features_model/vggt/extract_multi_frame_vggt_features_wrapper.py",
            "--dataset_root",
            args.rgb_root,
            "--output_root",
            model_out_root,
        ...
        cmd = [
            "python",
            "/home/gl/RoboTwin/policy/DP2DP3/features_model/dinov3/extract_multi_frame_dinov3_features_local.py",
            "--dataset_root",
            args.rgb_root,
            "--output_root",
            model_out_root,
        ...
        cmd = [
            "python",
            "/home/gl/RoboTwin/policy/DP2DP3/features_model/Depth-Anything-3/extract_multi_frame_depthanything3_features.py",
            "--rgb_root",
            args.rgb_root,
            "--out_root",
            model_out_root,
